...
public class _733_floodFill {
    ...
    private static final int[][] dic = new int[][] { { 0, -1 }, { -1, 0 }, { 1, 0 }, { 0, 1 } };
    ...
    public static int[][] floodFill(int[][] image, int sr, int sc, int color) {
        ...
        used[sr][sc] = Boolean.TRUE;
        dfs(image, sr, sc, color, used);

        return image;
    }

    public static void dfs(int[][] image, int sr, int sc, int color, boolean[][] used) {
        if (oldVal == image[sr][sc]) {
            image[sr][sc] = color;
        }
        for (int[] d : dic) {
            int newX = sr + d[0];
            int newY = sc + d[1];
            if (!isInArea(newX, newY) || image[newX][newY] != oldVal || used[newX][newY]) {
                continue;
            }

            used[newX][newY] = Boolean.TRUE;
            image[sr][sc] = color;
            dfs(image, newX, newY, color, used);
...
//        int[][] res = new int[][] { { 2, 2, 2 }, { 2, 2, 0 }, { 2, 0, 1 } };
        ...
        int sc2 = 0;
        int newColor2 = 0;
        int[][] result = _733_floodFill.floodFill(image2, sr2, sc2, newColor2);
        System.out.println();
    }
}
